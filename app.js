...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function calculateMACD(data, fastPeriod = 12, slowPeriod = 26, signalPeriod = 9) {
        ...
        const signalLinePadded = Array(macdLine.length - signalLine.length).fill(null).concat(signalLine);
    ...
    function analyzeMarket(data) {
        ...
        if (latestData.close > latestData.sma20 && latestData.sma20 > latestData.sma50 && latestData.rsi > 50) {
            sentiment = "bullish";
            sentimentDescription = "Multiple indicators suggest a bullish trend. Price is above key moving averages with positive momentum.";
        } else if (latestData.close < latestData.sma20 && latestData.sma20 < latestData.sma50 && latestData.rsi < 50) {
            sentiment = "bearish";
            sentimentDescription = "Multiple indicators suggest a bearish trend. Price is below key moving averages with negative momentum.";
        } else {
    ...
    function generateSignals(data) {
        ...
        if (data.length > 2 && data[data.length - 2].macd < data[data.length - 2].macdSignal && 
    ...
    function renderIndicators(container, data) {
        ...
        indicators.forEach(indicator => {
            ...
            badgeText.textContent = indicator.interpretation.type.charAt(0).toUpperCase() + indicator.interpretation.type.slice(1);
