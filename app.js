...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadData() {
    ...
    try {
      ...
      // Generate mock data (since we can't access Binance API due to CORS)
      const data = generateMockData(symbol, timeframe)

      // Show CORS alert since we're using mock data
      corsAlert.style.display = "flex"
      ...
      // Create new chart
      priceChart = createPriceChart(priceChartCanvas, data)
  ...
  function generateSignals(data) {
    ...
    if (data[data.length - 2].rsi < 30 && latestData.rsi > 30) {
      signals.push({
        type: "buy",
    ...
    if (data[data.length - 2].rsi > 70 && latestData.rsi < 70) {
      signals.push({
        type: "sell",
        time: latestData.time,
        price: latestData.close,
    ...
    if (data[data.length - 2].macd < data[data.length - 2].macdSignal && latestData.macd > latestData.macdSignal) {
      signals.push({
        type: "buy",
        time: latestData.time,
        price: latestData.close,
    ...
    if (data[data.length - 2].sma20 < data[data.length - 2].sma50 && latestData.sma20 > latestData.sma50) {
      signals.push({
        type: "buy",
        time: latestData.time,
        price: latestData.close,
        ...
        strength: 80,
        description: "SMA 20 crossed above SMA 50, indicating a bullish trend change.",
        targetPrice: latestData.close * 1.03, // 3% target
        stopLoss: latestData.close * 0.98, // 2% stop loss
        potentialRoi: 3,
      })
    ...
    if (data[data.length - 2].sma20 > data[data.length - 2].sma50 && latestData.sma20 < latestData.sma50) {
      signals.push({
        ...
        indicator: "SMA Crossover",
        strength: 80,
        description: "SMA 20 crossed below SMA 50, indicating a bearish trend change.",
        targetPrice: latestData.close * 0.97, // 3% target
        stopLoss: latestData.close * 1.02, // 2% stop loss
        potentialRoi: 3,
      })
  ...
  function createPriceChart(canvas, data) {
    ...
    const upperBand = data.map((d) => d.bbandsUpper)
    const lowerBand = data.map((d) => d.bbandsLower)
  ...
  function renderSignals(container, signals) {
    ...
    signals.forEach((signal) => {
      ...
      timeElement.textContent = new Date(signal.time).toLocaleString()
      ...
      entryPriceValue.textContent = signal.price.toFixed(2)
      ...
      targetPriceValue.textContent = signal.targetPrice.toFixed(2)
      ...
      stopLossValue.textContent = signal.stopLoss.toFixed(2)
      ...
      strengthContainer.className = "signal-strength"

      const progressContainer = document.createElement("div")
      progressContainer.className = "progress-container"

      const progressBar = document.createElement("div")
      progressBar.className = `progress-bar ${signal.strength > 70 ? "strong" : signal.strength < 30 ? "weak" : ""}`
      progressBar.style.width = `${signal.strength}%`

      progressContainer.appendChild(progressBar)

      const strengthLabel = document.createElement("div")
      strengthLabel.className = "signal-strength-label"
      ...
      strengthLabel.appendChild(weakLabel)
      strengthLabel.appendChild(strongLabel)

      strengthContainer.appendChild(progressContainer)
      strengthContainer.appendChild(strengthLabel)

      // Signal Description
      const descriptionContainer = document.createElement("div")
      descriptionContainer.className = "signal-description"
